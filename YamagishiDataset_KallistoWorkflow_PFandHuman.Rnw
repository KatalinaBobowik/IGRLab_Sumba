...
Read in Human count data and organise sample information:
...
Read in PF count data (*Note: there are duplicates in the kallisto counts data file):
...
Plotting and association functions from Irene:
<<tidy=TRUE,highlight=FALSE>>=
